...
namespace AST
{
	...
	TEST_CASE("ParserTest", "[ParserTest]")
	{
		...
		{
			ASTParser::ParseResult result = parserTest.mParser.ExtractOperator("123", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 0);
			REQUIRE(result.mErrorType == ASTParser::ResultType::InvalidOperator);
			result = parserTest.mParser.ExtractOperator("+", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 1);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("-", ASTNode::NodeType::Irrational);
			REQUIRE(result.mExtractedLength == 1);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("*", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 0);
			REQUIRE(result.mErrorType == ASTParser::ResultType::InvalidOperator);
			result = parserTest.mParser.ExtractOperator("/", ASTNode::NodeType::Rational);
			REQUIRE(result.mExtractedLength == 1);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("++", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 1);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("sinhabc", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 4);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("cosabc", ASTNode::NodeType::Operator);
			REQUIRE(result.mExtractedLength == 3);
			REQUIRE(result.mErrorType == ASTParser::ResultType::NoError);
			result = parserTest.mParser.ExtractOperator("taa", ASTNode::NodeType::Operator);
			REQUIRE(result.mErrorType == ASTParser::ResultType::InvalidOperator);
		}
